#include "include/device_sentinel_windows/device_sentinel_windows.h"
...
class DeviceSentinelWindows : public flutter::Plugin {
 public:
  static void RegisterWithRegistrar(
      flutter::PluginRegistrarWindows* registrar);

  DeviceSentinelWindows();
  virtual ~DeviceSentinelWindows();
 ...
 private:
  ...
  std::unique_ptr<device_sentinel::VolumeKeyDetector> volume_detector_;
  std::unique_ptr<device_sentinel::PowerButtonDetector> power_detector_;
};

void DeviceSentinelWindows::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows* registrar) {
  auto method_channel =
      std::make_unique<flutter::MethodChannel<EncodableValue>>(
          registrar->messenger(), "device_sentinel_windows",
          &flutter::StandardMethodCodec::GetInstance());

  auto event_channel =
      std::make_unique<flutter::EventChannel<EncodableValue>>(
          registrar->messenger(), "device_sentinel_windows/events",
          &flutter::StandardMethodCodec::GetInstance());

  auto plugin = std::make_unique<DeviceSentinelWindows>();
...
DeviceSentinelWindows::DeviceSentinelWindows() {}

DeviceSentinelWindows::~DeviceSentinelWindows() { StopDetection(); }

void DeviceSentinelWindows::HandleMethodCall(
    const flutter::MethodCall<EncodableValue>& method_call,
    std::unique_ptr<flutter::MethodResult<EncodableValue>> result) {
...
void DeviceSentinelWindows::StartDetection(bool intercept_volume) {
  StopDetection();

  volume_detector_ = std::make_unique<device_sentinel::VolumeKeyDetector>(
      intercept_volume,
  ...
  power_detector_ = std::make_unique<device_sentinel::PowerButtonDetector>(
      [this](const std::map<std::string, std::string>& event) {
        SendEvent(event);
      });
...
void DeviceSentinelWindows::StopDetection() {
  volume_detector_.reset();
  power_detector_.reset();
}

void DeviceSentinelWindows::SendEvent(
...
void DeviceSentinelWindowsRegisterWithRegistrar(
    FlutterDesktopPluginRegistrarRef registrar) {
  DeviceSentinelWindows::RegisterWithRegistrar(
